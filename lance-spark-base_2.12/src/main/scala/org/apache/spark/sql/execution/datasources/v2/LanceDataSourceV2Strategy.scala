...
case class LanceDataSourceV2Strategy(session: SparkSession) extends SparkStrategy
  ...
  override def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
    ...
    case LanceAnalyzeTable(ResolvedIdentifier(catalog, ident), columns, forAllColumns, approx) =>
      LanceAnalyzeTableExec(asTableCatalog(catalog), ident, columns, forAllColumns, approx) :: Nil
