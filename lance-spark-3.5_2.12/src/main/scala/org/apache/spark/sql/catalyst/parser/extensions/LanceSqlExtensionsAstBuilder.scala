...
class LanceSqlExtensionsAstBuilder(delegate: ParserInterface)
  ...
  override def visitStringLiteral(ctx: LanceSqlExtensionsParser.StringLiteralContext): String = {
    // Grammar permits `STRING+` (adjacent string tokens), so a literal like `'foo' 'bar'`
    // arrives as two STRING tokens that must each be unquoted independently before
    // concatenation. ctx.getText() naively concatenates raw token text, leaving inner quotes
    // (`"'foo''bar'"` → strip-prefix/suffix → `"foo''bar"`). Iterate and strip per-token.
