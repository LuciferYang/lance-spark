...
class LanceSparkSqlExtensionsParser(delegate: ParserInterface) extends ParserInterface {
  ...
  override def parsePlan(sqlText: String): LogicalPlan = {
    // Strip leading SQL trivia before the CREATE INDEX prefix check.
    ...
    if (LanceSparkSqlExtensionsParser.isLanceExtensionPriorityCommand(sqlText)) {
      ...
      // command. Delegating to Spark's parser would re-trigger the V2-rejection failure mode
      // the priority routing was created to prevent — let the Lance grammar error surface.
